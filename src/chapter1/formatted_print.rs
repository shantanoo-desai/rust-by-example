...
pub fn call_formatted() {
    ...
    println!("Named Arguments: {subject} {verb} {object}",
            object="the lazy dog",
    ...
    println!("White Space Padding: {number:>padding$}", number=1, padding=6);
    println!("Padding with Zeroes: {number:>0padding$}", number=1, padding=6);

    // Precision Control

    println!("value of {pi} till {precision} precisions is: {pi:.precision$}", pi=3.141592, precision=2);
}